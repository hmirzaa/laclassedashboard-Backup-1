...
import InputIcon from '@material-ui/core/SvgIcon/SvgIcon';
    ...
    >
      <nav className={classes.navigation}>
        {navConfig.map((list) => renderNavItems({
          items: list.items.map((item) => { return {...item, title: t(item.title)}}),
          subheader: t(list.subheader),
      ...
      <div className={classes.profile}>
        ...
        <div className={classes.details}>
          <Link
            ...
            color="textPrimary"
            underline="none"
            className={classes.fullNameColor}
          >
            {`${user.userData.fullName}`}
          </Link>
          <Typography variant="body2">{user.userData.email}</Typography>
        </div>
        {/*
        <IconButton
